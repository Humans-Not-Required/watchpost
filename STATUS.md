...
- Test suite: **387 Rust tests passing** (371 HTTP + 16 unit) + **266 Python SDK integration tests** — parallel-safe. SDK tests cover: constructor variants (env var, trailing slash, timeout), unicode handling (CJK names/tags/groups, emoji, maintenance/notification/status pages), monitor response fields, timestamps lifecycle, multi-monitor isolation (notifications/maintenance/heartbeats/incidents scoped, cross-key rejection), 3-level dependency chain (circular detection, middle deletion), notification enable/disable lifecycle, multiple maintenance windows, alert rules (partial update, zero values), bulk create (10 monitors, mixed types), status page advanced (private, logo_url, custom domain), dual discovery paths (llms.txt, SKILL.md, OpenAPI), heartbeat pagination, uptime structure, SLA structure/validation, export advanced, tags/groups detail, pause/resume lifecycle (idempotent), full monitor lifecycle (create→configure→pause→resume→export→delete), cross-feature interactions (SLA+uptime, status page+monitors, badge+state, dashboard counts, tags reflection), error response format (body structure, human-readable messages), SSE event construction, delete verification (wrong key, double delete), plus all original coverage: advanced monitor updates, cursor pagination, combined filters, status page lifecycle, dashboard auth variants, admin verify, settings auth, location/probe auth, alert rule validation, dependency advanced, maintenance edge cases, notification types, webhook delivery filters, export/import roundtrip, bulk create edge cases, incident/note not-found handling, badge variants, delete cascade, error hierarchy
